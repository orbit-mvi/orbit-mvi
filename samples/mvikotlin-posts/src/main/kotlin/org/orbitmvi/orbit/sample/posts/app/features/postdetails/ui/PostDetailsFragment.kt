...
import org.orbitmvi.orbit.sample.posts.app.features.postdetails.viewmodel.PostDetailsView
import org.orbitmvi.orbit.sample.posts.databinding.PostDetailsFragmentBinding

class PostDetailsFragment : Fragment(R.layout.post_details_fragment), PostDetailsView {
    ...
    private val controller by inject<PostDetailsController> {
        @Suppress("EXPERIMENTAL_API_USAGE")
        parametersOf(lifecycle.asMviLifecycle(), args.overview, getParcelableStateKeeperRegistry().get())
    }
    ...
    override fun render(model: PostDetailState) {
        if (!initialised) {
            initialised = true
            (activity as AppCompatActivity?)?.supportActionBar?.apply {
                title = model.postOverview.username
                Glide.with(requireContext()).load(model.postOverview.avatarUrl)
            ...
            binding.postTitle.text = model.postOverview.title
        }

        if (model is PostDetailState.Details) {
            binding.postBody.text = model.post.body

            val comments = model.post.comments.size
            ...
            adapter.update(model.post.comments.map(::PostCommentItem))
        }
    }
}
