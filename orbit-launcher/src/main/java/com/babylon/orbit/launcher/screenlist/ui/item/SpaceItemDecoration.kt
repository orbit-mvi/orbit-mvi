...
internal class SpaceItemDecoration(
    ...
    private fun assignItemOffsets(
        outRect: Rect,
        isFirstItem: Boolean,
        isLastItem: Boolean
    ) {
        outRect.left =
            if (!isHorizontal || isFirstItem) horizontalSpacing else horizontalSpacing / 2
        outRect.right =
            if (!isHorizontal || isLastItem) horizontalSpacing else horizontalSpacing / 2
